...
- [x] **Phase 2-C** — inference 서버 spawn 관리: 엔진 dropdown(xLLM/vLLM/llama-server/TabbyML/TabbyAPI/Ollama/Custom) + 받은 모델 dropdown + 바이너리/스크립트 경로 picker(PATH 의존 제거) + 자동 명령 합성 + child process 관리(시작/중지/로그/auto ping)
...
- [x] **Phase 2-10** — PTY 터미널: portable-pty(ConPTY/POSIX) + 라인 입력 모드 + ANSI strip + 하단 토글 패널(Ctrl+\`)
...
현재 검증 기준은 `cargo test --all-targets -- --test-threads=1`입니다. Windows ConPTY 및 외부 프로세스 fixture의 재현성을 위해 테스트 프로세스는 단일 스레드로 실행합니다. 2026-08-18 검증에서 unit test 639개가 통과하고 11개가 무시되었으며(Linux POSIX PTY Ctrl+C 수동 QA 항목 포함), external integration smoke test 1개가 통과했습니다.
...
TabbyML / TabbyAPI / xLLM / vLLM / llama-server / Ollama 등 OpenAI 호환 inference endpoint면 무엇이든 등록 가능. Settings → "OpenAI 호환 endpoint" 섹션에:
