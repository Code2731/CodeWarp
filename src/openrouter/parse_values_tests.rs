// openrouter/parse_values_tests.rs — Tests for parse_values (openrouter child module)
...
fn nested_object_content_via_value_to_text() {
    let v = json!({"choices": [{"message": {"content": [{"text": "Hello"}, {"text": "World"}]}}]});
    assert_eq!(
        extract_non_stream_content_from_value(&v).as_deref(),
        Some("HelloWorld")
    );
...
fn top_level_precedence_after_choices() {
    let v =
        json!({"choices": [{"message": {"content": "from_choices"}}], "output_text": "from_top"});
    assert_eq!(
        extract_non_stream_content_from_value(&v).as_deref(),
        Some("from_choices")
    );
