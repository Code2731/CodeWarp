...
impl App {
    ...
    fn view_stream(&self) -> Element<'_, Message> {
        ...
        let mode_label = button(text(self.agent_mode.label()).size(FS_LABEL).font(semibold_font()))
            ...
            if filtered.is_empty() {
                ...
                for (i, path) in filtered.iter().enumerate() {
                    ...
                    let btn = button(text(label).size(FS_BODY))
                        ...
                        .padding([6, 10])
                        .width(Length::Fill)
                        .style(if is_selected { primary_btn } else { secondary_btn });
                    list = list.push(btn);
                }
                container(
                    scrollable(list)
                ...
                .style(|theme: &Theme| {
                    ...
                    container::Style {
                        background: Some(p.background.strong.color.into()),
                        border: iced::Border { color: p.primary.base.color, width: 1.0, radius: 10.0.into() },
                        ..Default::default()
                    }
                })
                .into()
    ...
    fn view_settings(&self) -> Element<'_, Message> {
        ...
        let local_model_count = list_downloaded_models(std::path::Path::new(&self.model_dir_input)).len();
        ...
        let tab_btn = |icon: &'static str, label: &'static str, note: String, health: TabHealth, tab: SettingsTab| {
        ...
        let actions = row![
            button(text("저장").size(FS_SUBTITLE)).on_press_maybe(
            ...
            button(text("삭제").size(FS_SUBTITLE)).on_press_maybe(
        ...
        let tabby_actions = row![
            button(text("저장").size(FS_SUBTITLE)).on_press_maybe(if self.busy {
                None
            } else {
                Some(Message::SaveTabby)
            }).style(primary_btn),
            button(text("연결 테스트").size(FS_SUBTITLE)).on_press_maybe(
        ...
        let tabs = row![
            ...
            tab_btn(
                "◇",
                "MCP",
                format!("{} srv / {} tools", self.mcp_servers.len(), self.mcp_tools.len()),
                mcp_health,
                SettingsTab::Mcp
            ),
        ]
        ...
        let summary = row![
            text(format!(
                ...
                if self.inference_pid.is_some() { "running" } else { "stopped" },
        ...
        let (active_tab_title, active_health, active_action, quick_label, quick_action) = match self.settings_tab {
            SettingsTab::Provider => (
                "Provider",
                provider_health,
                if self.has_key || !self.tabby_url_input.trim().is_empty() {
                    ...
                    "권장 액션: OpenRouter 키 저장 또는 로컬 endpoint URL을 먼저 등록해 주세요.".to_string()
                },
            ...
            SettingsTab::Runtime => (
                ...
                runtime_health,
                if self.inference_pid.is_some() {
                    "권장 액션: 현재 로그를 확인하고 필요한 경우 중지 후 모델을 교체하세요.".to_string()
                } else {
                    "권장 액션: 엔진/모델(또는 커스텀 명령) 입력 후 시작 버튼을 눌러주세요.".to_string()
                },
                if self.inference_pid.is_some() { "중지" } else { "시작" },
                if self.inference_pid.is_some() {
            ...
            SettingsTab::Models => (
                ...
                if local_model_count > 0 { "Runtime 탭으로" } else { "기본 EXL2 다운로드" },
                if local_model_count > 0 {
            ...
            SettingsTab::Mcp => (
                ...
                if self.mcp_servers.is_empty() {
                    "권장 액션: 서버 이름과 명령을 입력해 MCP 서버를 하나 추가해 주세요.".to_string()
                } else if self.mcp_tools.is_empty() {
                    "권장 액션: 서버 명령이 유효한지 확인하고 tools 로드를 기다려 주세요.".to_string()
                } else {
                    "권장 액션: 채팅에서 MCP 도구 호출이 정상 동작하는지 점검해 주세요.".to_string()
                },
                "서버 추가",
                if !self.mcp_name_input.trim().is_empty() && !self.mcp_command_input.trim().is_empty() {
    ...
    fn view_inference_runner(&self) -> Element<'_, Message> {
        ...
        let model_section: Element<Message> = match self.inference_engine {
            InferenceEngine::XLlm | InferenceEngine::VLlm | InferenceEngine::LlamaServer => {
                ...
                if models.is_empty() {
                    container(
                        text("받은 모델 없음 — Models 탭에서 먼저 다운로드").size(FS_LABEL),
                    )
                    .padding([8, 10])
                    .style(panel_style)
                    .into()
        ...
        container(column![
            header,
            row![text("엔진").size(FS_LABEL).font(semibold_font()), engine_pick]
                .spacing(8)
                .align_y(Alignment::Center),
            binary_section,
            model_section,
    ...
    fn view_mcp_settings(&self) -> Element<'_, Message> {
        ...
        for (i, s) in self.mcp_servers.iter().enumerate() {
            ...
            server_list = server_list.push(
                row![
                    text(shorten_tail(&label, 72)).size(FS_BODY).width(Length::Fill),
                    button(text("✕").size(FS_LABEL))
        ...
        container(column![header, hint, empty_state, server_list, add_row]
            .spacing(8))
            .padding([14, 16])
            .width(Length::Fill)
            .style(panel_style)
    ...
    fn view_model_manager(&self) -> Element<'_, Message> {
        ...
        let local_state: Element<Message> = if local_model_count == 0 {
            container(text("로컬 모델이 비어 있습니다. 프리셋 1개를 먼저 내려받으세요.").size(FS_LABEL))
                .padding([8, 10])
                .style(panel_style)
                .into()
        } else {
            container(
                text(format!("로컬 모델 {}개가 준비되어 있습니다.", local_model_count)).size(FS_LABEL),
            )
            .padding([8, 10])
            .style(panel_style)
            .into()
        ...
        let mut exl2_col = column![
            text("EXL2 프리셋 (TabbyAPI용) — 클릭하면 바로 다운로드").size(FS_BODY),
        ]
        .spacing(4);
        for (i, p) in EXL2_PRESETS.iter().enumerate() {
            ...
            exl2_col = exl2_col.push(if is_downloading {
                ...
                btn.on_press(Message::DownloadExl2Preset(i)).style(secondary_btn)
            });
        }
        ...
        container(column![
            ...
            Space::new().height(Length::Fixed(4.0)),
            text("저장 경로 (변경 가능)").size(FS_LABEL).font(semibold_font()),
            dir_row,
            Space::new().height(Length::Fixed(12.0)),
            exl2_col,
            Space::new().height(Length::Fixed(12.0)),
            text("HF 일반 모델 (safetensors · xLLM/vLLM용) — 클릭 → 입력란에 채움").size(FS_BODY),
            presets_col,
            Space::new().height(Length::Fixed(8.0)),
